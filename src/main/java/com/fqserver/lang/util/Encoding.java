...
import com.fqserver.lang.exceptions.UnsupportedException;
...
public class Encoding {
    ...
    public static Encoding create(String name) {
        try {
            ...
            System.err.println(String.format("error create encoding '%s', unsupported charset error",
                                             name));
            e.printStackTrace();
    ...
    public byte[] getBytes(String string) {
        try {
            return null == string ? new byte[0] : string.getBytes(charset.name());
        }
    ...
    public String getString(byte[] bytes) {
        try {
            return null == bytes || bytes.length == 0 ? "" : new String(bytes, charset.name());
        }
        catch (UnsupportedEncodingException e) {
            throw new UnsupportedException(e.getMessage(), e);
        }
    ...
    private static final class UnsupportedEncoding extends Encoding {
        ...
        private static final String message = "unsupported charset '%s'";
