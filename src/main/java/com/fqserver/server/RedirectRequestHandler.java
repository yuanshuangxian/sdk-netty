...
public class RedirectRequestHandler extends RequestHandler {
    ...
    protected void writeResponse(final byte[] data) {
        writeResponse(HttpResponseStatus.FOUND, data);
    }
     ...
     */
    @Override
    public void write(Map<String, Object> responseMap) {

        try {
            if (cContentType == ContentTypes.CBOR) {
                writeResponse(CborUtil.objectToJsonByte(sendMap));
            } else if (cContentType == ContentTypes.JSON) {
                writeResponse(JsonUtil.objectToJsonByte(sendMap));
            } else if (cContentType == ContentTypes.BINARY) {
                writeResponse(JsonUtil.objectToJson(sendMap).getBytes(cContentCharset));
            } else if (cContentType == ContentTypes.PLAIN) {
                writeResponse(JsonUtil.objectToJson(sendMap).getBytes(cContentCharset));
            }
        }
    ...
    public void write(String content, String localtion) {
    ...
    protected void afterBuildResponse(final HttpResponseStatus status,
        ...
        if (HttpResponseStatus.FOUND.equals(status))
            response.headers().set("location", localtion);
        if (null == bean) {
            return;
        }
